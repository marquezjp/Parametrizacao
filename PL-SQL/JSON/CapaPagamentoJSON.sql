select * from table(listar(CapaPagamento()));
/

select mostrar(CapaPagamento()) from dual;
/

select json_query(CapaPagamento(), '$' pretty ) from dual;
/

select json_serialize(CapaPagamento() returning varchar2 pretty ) from dual;
/

drop function CapaPagamento;
drop function mostrar;
drop function listar;
drop type layoutTable;
drop type layoutRow;
/

create or replace type layoutRow as object (
...
create or replace type layoutTable as table of layoutRow;
/
...
as
begin
 return json_query(CapaPagamento(), '$' returning clob pretty);
end mostrar;
/

create or replace
function listar(in_json in clob) return layoutTable
as
 v_ret   layoutTable;
begin
 select layoutRow(
...
--  sigrh_coluna
 ...
 from json_table(in_json, '$' columns (
