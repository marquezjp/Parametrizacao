...
CREATE OR REPLACE PACKAGE BODY PKGMIG_ParametrizacaoEventosPagamento AS
  ...
  -- FUNCTION: pExportar
  -- Objetivo:
  --   Exportar as Parametrizações dos Eventos de Pagamento para a Configuração Padrão JSON,
  --     realizando:
  --     - Gera o Documento JSON EventosPagamento
  ...
  -- ###########################################################################
    ...
    BEGIN
      ...
      ELSE
        vtxMensagem := 'Inicio da Exportação da Parametrização do Evento de Pagamento "' || pcdIdentificacao || '" ';
      END IF;
      ...
      END IF;
	    ...
	    -- Defini o Cursos com a Query que Gera o Documento JSON Eventos de Pagamento
	    vRefCursor := fnCursorEventos(psgAgrupamento, pcdIdentificacao);
      ...
      -- Loop principal de processamento
        ...
        PKGMIG_ParametrizacaoLog.pAlertar('Exportação do Evento de Pagamento ' || rcdIdentificacao,
          cAUDITORIA_DETALHADO, pnuNivelAuditoria);
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ...
        PKGMIG_ParametrizacaoLog.pRegistrarErro(psgAgrupamento, vsgOrgao, ctpOperacao, vdtOperacao,  
          csgModulo, csgConceito, vcdIdentificacao, 'EVENTO PAGAMENTO',
          'Exportação dos Eventos (PKGMIG_ParametrizacaoEventosPagamento.pExportar)', SQLERRM);
      ROLLBACK;
      RAISE;
  END fnExportar;
  ...
  -- PROCEDURE: pImportar
  -- Objetivo:
  --   Importar dados Eventos de Pagamento partir do Documento Rubrica JSON
  --   contida na tabela emigParametrizacao, realizando:
  --     - Importação dos Eventos de Pagamento para tabela epagEventoPagAgrup
  --     - Registro de Logs de Auditoria por evento
  --
  -- Parâmetros:
  ...
  -- ###########################################################################
    ...
    -- Cursor que extrai e transforma os dados JSON de Eventos de Pagamento
    CURSOR cDados IS
      WITH
      -- Eventos de Pagamento das Parametrizações das Rubricas
      Eventos AS (
  ...
  BEGIN
    ...
    IF pcdIdentificacao IS NULL THEN
      vtxMensagem := 'Inicio da Importação das Parametrizações dos Eventos de Pagamento ';
    ELSE
      vtxMensagem := 'Inicio da Importação da Parametrização do Evento de Pagamento "' || pcdIdentificacao || '" ';
    END IF;
    ...
    FOR r IN cDados LOOP
      ...
      IF r.cdRubricaAgrupamento IS NULL THEN
        PKGMIG_ParametrizacaoLog.pAlertar('Importação dos Eventos de Pagamento - ' ||
          'Evento de Pagamento - ' || vcdIdentificacao ||
          'Rubrica do Evento de Pagamento Inexistente no Agrupamento',
          cAUDITORIA_ESSENCIAL, pnuNivelAuditoria);
        ...
        PKGMIG_ParametrizacaoLog.pRegistrar(psgAgrupamentoDestino, vsgOrgao, ctpOperacao, vdtOperacao, 
          ...
          'EVENTO PAGAMENTO', 'INCONSISTENTE',
          'Rubrica do Evento de Pagamento Inexistente no Agrupamento',
          cAUDITORIA_ESSENCIAL, pnuNivelAuditoria);
      END IF;
      ...
      IF r.cdRubricaAgrupamento IS NOT NULL THEN
        ...
        -- Incluir Eventos de Pagamento
        vContador := vContador + 1;
        vnuInseridos := vnuInseridos + 1;
        pImportarEventos(psgAgrupamentoDestino, vsgOrgao, ctpOperacao, vdtOperacao,
          csgModulo, csgConceito, vcdIdentificacao, r.cdRubricaAgrupamento, r.Eventos, pnuNivelAuditoria);
    ...
    -- Gerar as Estatísticas da Importação dos Eventos de Pagamento
    ...
    vtxResumo := 
      ...
      'Data e Hora da Termino da Operação ' || TO_CHAR(vdtTermino, 'DD/MM/YYYY HH24:MI:SS')  || ', ' || CHR(13) || CHR(10) ||
	    ...
	    'Total de Parametrizações dos Eventos de Pagamento Incluídos: ' || vnuInseridos;
    ...
    PKGMIG_ParametrizacaoLog.pRegistrar(psgAgrupamentoDestino, vsgOrgao, ctpOperacao, vdtOperacao,
      ...
      'EVENTO PAGAMENTO', 'RESUMO', 'Importação das Parametrizações dos Eventos de Pagamento do ' || vtxResumo, 
      cAUDITORIA_ESSENCIAL, pnuNivelAuditoria);

    -- Atualizar a SEQUENCE das Tabela Envolvidas na importação dos Eventos de Pagamento
    ...
    PKGMIG_ParametrizacaoLog.pAlertar('Termino da Importação das Parametrizações dos Eventos de Pagamento do ' ||
    ...
    WHEN OTHERS THEN
      -- Registro e Propagação do Erro
        PKGMIG_ParametrizacaoLog.pRegistrarErro(psgAgrupamentoDestino, vsgOrgao, ctpOperacao, vdtOperacao,  
          csgModulo, csgConceito, vcdIdentificacao, 'EVENTO PAGAMENTO',
          'Importação dos Eventos (PKGMIG_ParametrizacaoEventosPagamento.pImportar)', SQLERRM);
    ROLLBACK;
    RAISE;
  END pImportar;
  ...
  -- ###########################################################################
    ...
    CURSOR cDados IS
      ...
      JSON_SERIALIZE(TO_CLOB(js.VigenciasEvento) RETURNING CLOB) AS VigenciasEvento
        ...
        -- $.PAG.Rubrica.Tipos[*].Agrupamento.Eventos
    ...
    EXCEPTION
      ...
      -- Registro e Propagação do Erro
        PKGMIG_ParametrizacaoLog.pRegistrarErro(psgAgrupamentoDestino, psgOrgao, ptpOperacao, pdtOperacao,  
          ...
          'Importação dos Eventos (PKGMIG_ParametrizacaoEventosPagamento.pImportarEventos)', SQLERRM);
      RAISE;
  END pImportarEventos;
  ...
  -- ###########################################################################
    ...
    CURSOR cDados IS
      ...
      -- Caminho Absoluto no Documento JSON
      -- $.PAG.Rubrica.Tipos[*].Agrupamento.Eventos.Vigencias[*]
  ...
  -- Função que cria o Cursor que Estrutura o Documento JSON com as Parametrizações dos Eventos de Pagamento
  ...
  BEGIN
    OPEN vRefCursor FOR
      --- Extrair os Conceito de Eventos de Pagamento das Rubricas de um Agrupamento
      WITH
      --- Informações referente as lista de Órgãos, Rubricas, Carreiras, Cargos Comissionados, Motivos
      -- OrgaoLista: lista dos Agrupamentos e Órgãos
      OrgaoLista AS (
      ...
      Evento AS (
        SELECT evento.cdAgrupamento, rub.nuRubrica,
          JSON_ARRAYAGG(JSON_OBJECT(
