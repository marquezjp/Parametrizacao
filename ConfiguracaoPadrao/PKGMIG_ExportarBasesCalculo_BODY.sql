...
CREATE OR REPLACE PACKAGE BODY PKGMIG_ExportarBasesCalculo AS
  ...
  BEGIN
    ...
    PKGMIG_ConfiguracaoPadrao.PConsoleLog('Inicio da Exportações das Bases do Agrupamento ' || psgAgrupamento);

    -- Defini o Cursos com a Query que Gera o Documento JSON ValoresReferencia
    vRefCursor := fnCursorBases(psgAgrupamento);
    
    -- Loop principal de processamento
    LOOP
      FETCH vRefCursor INTO vsgAgrupamento, vsgOrgao, vsgModulo, vsgConceito, vcdIdentificacao, vjsConteudo;
      ...
      INSERT INTO emigConfiguracaoPadrao (
        sgAgrupamento, sgOrgao, sgModulo, sgConceito, cdIdentificacao, jsConteudo, dtInclusao, nuVersao, flAnulado
      ) VALUES (
        vsgAgrupamento, vsgOrgao, vsgModulo, vsgConceito, vcdIdentificacao, vjsConteudo,
        SYSTIMESTAMP, '1.00', 'N'
      );
      ...
      PKGMIG_ConfiguracaoPadrao.pRegistrarLog(psgAgrupamento, vsgOrgao,
        ...
        'INSERT BASE', 'Documento JSON Bases incluidos com sucesso',
    ...
    vsgModulo, vsgConceito, Null,
      'RESUMO', 'Bases incluidas: ' || vnuInseridos,
      cDEBUG_DESLIGADO, pnuDEBUG);

    COMMIT;

    PKGMIG_ConfiguracaoPadrao.PConsoleLog('Resumo da Exportações das Bases do Agrupamento: ' ||
      'Bases incluidas: ' || vnuInseridos);
    
    PKGMIG_ConfiguracaoPadrao.PConsoleLog('Termino da Exportações das Bases do Agrupamento ' || psgAgrupamento);
  ...
  FUNCTION fnCursorBases(psgAgrupamento IN VARCHAR2) RETURN SYS_REFCURSOR IS
    vRefCursor SYS_REFCURSOR;
  BEGIN
    OPEN vRefCursor FOR
    ...
    */
    ...
    SELECT blexp.cdBaseCalculoBlocoExpressao, blexp.cdBaseCalculoBloco, mneu.sgTipoMneumonico,
      JSON_OBJECT(
        ...
        'nuRubrica'             VALUE CASE WHEN rub.nuRubrica IS NULL THEN NULL ELSE LPAD(tprub.nuTipoRubrica,2,0) || '-' || LPAD(rub.nuRubrica,4,0) END,
        ...
        'GrupoRubricas'         VALUE CASE WHEN json_exists(gruporub.GrupoRubricas, '$.*') THEN NULL ELSE gruporub.GrupoRubricas END
    ...
    SELECT vigencia.cdVersaoBaseCalculo,
      JSON_ARRAYAGG(JSON_OBJECT('Vigencia' VALUE JSON_OBJECT(
    ...
    SELECT a.sgAgrupamento, o.sgOrgao, base.sgBaseCalculo,
	  JSON_OBJECT('PAG' value JSON_OBJECT(
