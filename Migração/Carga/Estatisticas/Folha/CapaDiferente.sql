...
select
 ...
 o.sgorgao as Orgao,
 lpad(trim(nusequencialfolha - 12),2,0) as Seq,
 lpad(trim(capa.nucpf), 11, 0) as CPF,
 lpad(to_number(trim(replace(numatriculalegado,'"',''))),9,0) as MatriculaLegado,
 to_char(to_date(trim(capa.dtadmissao), 'DD/MM/YYYY'), 'DD/MM/YYYY') as DataAdmissao,
 '0000000-0-00' as Matricula,
...
where (to_number(nvl(replace(capa.vlproventos,'.',','), 0)) != 0 or to_number(nvl(replace(capa.vldescontos,'.',','), 0)) != 0)
  and lpad(capa.nuanoreferencia,4,0) = 2023 and lpad(capa.numesreferencia,2,0) = 10 and lpad(trim(nusequencialfolha),2,0) = 13
...
/*
select * from (
select * from capa union 
select * from capamig
)
where CPF = 43599788472;
*/

select count(1) from (
select capamig.AnoMes, capamig.Orgao, capamig.Seq, capamig.CPF, capamig.MatriculaLegado, capamig.DataAdmissao, capa.Matricula, capamig.Nome, capamig.Proventos, capamig.Descontos, capamig.Credito,
 case
  when capa.CPF is null then 'Capa de Pagamento para o CPF e Data de Admissão Inexistente'
  when capa.MatriculaLegado is null then 'Matricula Legada Inexistente no SIGRH'
  when capa.MatriculaLegado != capamig.MatriculaLegado then 'Matricula Legada Diferente no SIGRH'
  when capa.Proventos != capamig.Proventos then 'Total de Proventos Diferente'
  else 'Total de Proventos Igual'
 end as obs
from capamig
left join capa on capa.CPF = capamig.CPF and capa.MatriculaLegado = capamig.MatriculaLegado and capa.dataadmissao = capamig.dataadmissao
where capa.CPF is not null
  and capa.MatriculaLegado is not null and capa.MatriculaLegado = capamig.MatriculaLegado
  and capa.Proventos != capamig.Proventos
...
from capamig
left join capa on capa.CPF = capamig.CPF and capa.dataadmissao = capamig.dataadmissao
where capa.CPF is null
   or capa.MatriculaLegado is null
   or capa.MatriculaLegado != capamig.MatriculaLegado
)
--order by 4, 5, 6, 2
;
