...
select
 pag.nuanoreferencia, pag.numesreferencia,
 upper(trim(o.sgorgao)) as sgorgao,
 lpad(trim(pag.nusequencialfolha - 12),2,0) as nusequencialfolha,
 pag.nucpf, pag.numatriculalegado, pag.dtadmissao, pag.nmtiporubrica, pag.nurubrica, pag.nmrubrica, pag.vlpagamento
from sigrhmig.emigcontrachequecsv_202310 pag
...
where lpad(pag.nuanoreferencia,4,0) = 2023 and lpad(pag.numesreferencia,2,0) = 10 and lpad(trim(pag.nusequencialfolha),2,0) = 13
  and o.sgagrupamento = 'ADM-DIR'
  and upper(trim(pag.nmtiporubrica)) != 'BASE'
),
...
select 'LEGADO' as Origem,
 lpad(pag.nuanoreferencia,4,0) || lpad(pag.numesreferencia,2,0) as AnoMes,
 pag.sgorgao as Orgao,
 lpad(trim(pag.nusequencialfolha - 12),2,0) as Seq,
 lpad(trim(pag.nucpf), 11, 0) as CPF,
 lpad(to_number(trim(replace(pag.numatriculalegado,'"',''))),9,0) as MatriculaLegado,
 to_char(to_date(trim(pag.dtadmissao), 'DD/MM/YYYY'), 'DD/MM/YYYY') as DataAdmissao,
 case when m.numatriculalegado is null then '0000000-0-00' else lpad(m.numatricula,7,0) || '-' || m.nudvmatricula || '-' || lpad(trim(m.nuseqmatricula),2,0) end as Matricula,
 upper(trim(pag.nmtiporubrica)) as TipoRubrica,
 case
  when ra.cdagrupamento is null then 0
  when upper(trim(pag.nmtiporubrica)) = 'PROVENTOS NORMAL' and lpad(trim(pag.nurubrica),4,0) in (0001, 0002, 0181, 0524) then 1
  when upper(trim(pag.nmtiporubrica)) = 'DESCONTOS NORMAL' and lpad(trim(pag.nurubrica),4,0) in (0003, 0004, 0182) then 2
  when ra.flpensaoalimenticia = 'S' then 3
  when ra.flconsignacao = 'S' then 4
  else 9
 end CodigoGrupoRubrica,
 case
  when ra.cdagrupamento is null then 'RUBRICA NAO CADASTRADA'
  when upper(trim(pag.nmtiporubrica)) = 'PROVENTOS NORMAL' and lpad(trim(pag.nurubrica),4,0) in (0001, 0002, 0181, 0524) then 'VENCIMENTO'
  when upper(trim(pag.nmtiporubrica)) = 'DESCONTOS NORMAL' and lpad(trim(pag.nurubrica),4,0) in (0003, 0004, 0182) then 'TRIBUTOS'
 ...
 case
  when ra.cdagrupamento is null then 'RUBRICA NAO CADASTRADA'
  when upper(trim(pag.nmtiporubrica)) = 'PROVENTOS NORMAL' and lpad(trim(pag.nurubrica),4,0) in (0001, 0002, 0181, 0524) then upper(trim(pag.nmrubrica))
  when upper(trim(pag.nmtiporubrica)) = 'DESCONTOS NORMAL' and lpad(trim(pag.nurubrica),4,0) in (0003, 0004, 0182) then upper(trim(pag.nmrubrica))
  when ra.flconsignacao = 'S' then 'CONSIGNACAO' 
  when ra.flpensaoalimenticia = 'S' then 'PENSAO ALIMENTICIA'
  else 'OUTROS'
 end SubGrupoRubrica,
-- case upper(trim(pag.nmtiporubrica)) when 'PROVENTOS NORMAL' then 1 when 'DESCONTOS NORMAL' then 5 when 'TOTALIZADORES' then 9 else null end cdtiporubrica, 
 lpad(trim(pag.nurubrica),4,0) as Rubrica,
 upper(trim(pag.nmrubrica)) as DescricaoRubrica,
 to_number(nvl(replace(pag.vlpagamento,'.',','),0)) as Valor
from ContrachequeLegado pag
left join emigmatricula m on m.numatriculalegado = to_number(pag.numatriculalegado) and m.dtadmissao = to_date(pag.dtadmissao, 'DD/MM/YYYY')
left join epagrubrica r on r.cdtiporubrica = case upper(trim(pag.nmtiporubrica)) when 'PROVENTOS NORMAL' then 1 when 'DESCONTOS NORMAL' then 5 when 'TOTALIZADORES' then 9 else null end
                       and r.nurubrica = lpad(trim(pag.nurubrica),4,0)
left join epagrubricaagrupamento ra on ra.cdagrupamento = 1 and ra.cdrubrica = r.cdrubrica
left join epaghistrubricaagrupamento rub on rub.cdrubricaagrupamento = ra.cdrubricaagrupamento and nuanofimvigencia is null
),
...
select 'SIGRH' as Origem,
 ...
 case
  when r.cdtiporubrica = 1 and r.nurubrica in (0001, 0002, 0181, 0524) then 1
 ...
 case
  when r.cdtiporubrica = 1 and r.nurubrica in (0001, 0002, 0181, 0524) then 'VENCIMENTO'
 ...
 case
  when r.cdtiporubrica = 1 and r.nurubrica in (0001, 0002, 0181, 0524) then trim(replace(rub.derubricaagrupamento, '-',' '))
