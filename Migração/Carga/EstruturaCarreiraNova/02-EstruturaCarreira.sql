...
where (cef.sgorgao is null or o.cdorgao is not null)
  ...
  and cef.nmrelacaotrabalho in ('EFETIVO', 'ACT - ADMITIDO EM CARATER TEMPORARIO')
...
select distinct
 ...
 translate(regexp_replace(upper(trim(cef.degrupoocupacional)), '[[:space:]]+', chr(32)),
                 'ÁÉÍÓÚÀÈÌÒÙÂÊÎÔÛÃÕËÏÖÜÇÑŠÝŸŽåáéíóúàèìòùâêîôûãõëïöüçñšýÿž',
                 'AEIOUAEIOUAEIOUAOEIOUCNSYYZaaeiouaeiouaeiouaoeioucnsyyz') as deitemcarreira
from sigrh_rr_carreira_cargos cef
left join vcadorgao o on o.sgorgao = cef.sgorgao
...
select distinct
 ...
 translate(regexp_replace(upper(trim(cef.declasse)), '[[:space:]]+', chr(32)),
                 'ÁÉÍÓÚÀÈÌÒÙÂÊÎÔÛÃÕËÏÖÜÇÑŠÝŸŽåáéíóúàèìòùâêîôûãõëïöüçñšýÿž',
                 'AEIOUAEIOUAEIOUAOEIOUCNSYYZaaeiouaeiouaeiouaoeioucnsyyz') as deitemcarreira
from sigrh_rr_carreira_cargos cef
left join vcadorgao o on o.sgorgao = cef.sgorgao
...
select distinct
 ...
 translate(regexp_replace(upper(trim(cef.decompetencia)), '[[:space:]]+', chr(32)),
                 'ÁÉÍÓÚÀÈÌÒÙÂÊÎÔÛÃÕËÏÖÜÇÑŠÝŸŽåáéíóúàèìòùâêîôûãõëïöüçñšýÿž',
                 'AEIOUAEIOUAEIOUAOEIOUCNSYYZaaeiouaeiouaeiouaoeioucnsyyz') as deitemcarreira
from sigrh_rr_carreira_cargos cef
left join vcadorgao o on o.sgorgao = cef.sgorgao
left join ecadagrupamento a on a.cdagrupamento = o.cdagrupamento
where (cef.sgorgao is null or o.cdorgao is not null)
  and cef.decompetencia is not null
  and cef.nmrelacaotrabalho in ('EFETIVO', 'ACT - ADMITIDO EM CARATER TEMPORARIO')
...
select distinct
 ...
 translate(regexp_replace(upper(trim(cef.deespecialidade)), '[[:space:]]+', chr(32)),
                 'ÁÉÍÓÚÀÈÌÒÙÂÊÎÔÛÃÕËÏÖÜÇÑŠÝŸŽåáéíóúàèìòùâêîôûãõëïöüçñšýÿž',
                 'AEIOUAEIOUAEIOUAOEIOUCNSYYZaaeiouaeiouaeiouaoeioucnsyyz') as deitemcarreira
from sigrh_rr_carreira_cargos cef
left join vcadorgao o on o.sgorgao = cef.sgorgao
...
null as cdcargosirh,
case when i.nmtipoitemcarreira = 'CARREIRA' and i.deitemcarreira like upper('%POLICIAL%') then 3
     when i.nmtipoitemcarreira = 'CARREIRA' and i.deitemcarreira like upper('%SEED%') then 2
...
where nmdescricaoqlp like 'QUADRO%'
),
carreria_existe as (
select
 e.cdagrupamento,
...
--- Criar ecadEstruturaCarreira, incluir os Grupo Ocupacional na Carreira
--insert into ecadestruturacarreira
...
select
 ...
 ic.deitemcarreira as degrupoocupacional
from ecadestruturacarreira e 
inner join ecadagrupamento a on a.cdagrupamento = e.cdagrupamento
inner join ecaditemcarreira ic on ic.cdagrupamento = e.cdagrupamento and ic.cdtipoitemcarreira = 2 and ic.cditemcarreira = e.cditemcarreira
...
inner join carrerias carr on carr.sgagrupamento = g.sgagrupamento and carr.decarreira = g.decarreira
inner join ecaditemcarreira i on i.cdagrupamento = a.cdagrupamento
                             and i.cdtipoitemcarreira = 2
                             and i.deitemcarreira = g.degrupoocupacional
left join grupoocupacional_existe gexist on gexist.sgagrupamento = a.sgagrupamento
                                        and gexist.decarreira = g.decarreira
                                        and gexist.degrupoocupacional = g.degrupoocupacional
