define matricula = 52872;
define seq = 1;
define nivel = 'B';
define referencia = '02';
/

--- Lista o Último Cargo Efetivo do Vínculo
select * from ecadHistCargoEfetivo
where cdhistcargoefetivo = (select cef.cdhistcargoefetivo
from (select cdvinculo from ecadvinculo where numatricula = &matricula and nuseqmatricula = &seq) v
inner join (select cdvinculo, max(dtinicio) as dtinicio from ecadHistCargoEfetivo group by cdvinculo) ultimocef on ultimocef.cdvinculo = v.cdvinculo
inner join (select cdhistcargoefetivo, cdvinculo, dtinicio from ecadHistCargoEfetivo) cef on cef.cdvinculo = ultimocef.cdvinculo and cef.dtinicio = ultimocef.dtinicio
)
;
/

--- Lista Último Nível Referencia do Cargo Efetivo do Vínculo
select * from ecadHistNivelRefCEF
where cdhistnivelrefcef = (select nivref.cdhistnivelrefcef
from (select cdvinculo from ecadvinculo where numatricula = &matricula and nuseqmatricula = &seq) v
inner join (select cdvinculo, max(dtinicio) as dtinicio from ecadHistCargoEfetivo group by cdvinculo) ultimocef on ultimocef.cdvinculo = v.cdvinculo
inner join (select cdhistcargoefetivo, cdvinculo, dtinicio from ecadHistCargoEfetivo) cef on cef.cdvinculo = ultimocef.cdvinculo and cef.dtinicio = ultimocef.dtinicio
inner join (select cdhistcargoefetivo, max(dtinicio) as dtinicio from ecadHistNivelRefCEF group by cdhistcargoefetivo) ultimonivref on ultimonivref.cdhistcargoefetivo = cef.cdhistcargoefetivo and ultimonivref.dtinicio = cef.dtinicio
inner join (select cdhistnivelrefcef, cdhistcargoefetivo, dtinicio from ecadHistNivelRefCEF) nivref on nivref.cdhistcargoefetivo = ultimonivref.cdhistcargoefetivo and nivref.dtinicio = ultimonivref.dtinicio
);
/

--- Atualiza o Nível Referencia do Último Cargo Efetivo do Vínculo
update ecadHistCargoEfetivo
set nunivelpagamento = &nivel,
    nureferenciapagamento = &referencia
where cdhistcargoefetivo = (select cef.cdhistcargoefetivo
from (select cdvinculo from ecadvinculo where numatricula = &matricula and nuseqmatricula = &seq) v
inner join (select cdvinculo, max(dtinicio) as dtinicio from ecadHistCargoEfetivo group by cdvinculo) ultimocef on ultimocef.cdvinculo = v.cdvinculo
inner join (select cdhistcargoefetivo, cdvinculo, dtinicio from ecadHistCargoEfetivo) cef on cef.cdvinculo = ultimocef.cdvinculo and cef.dtinicio = ultimocef.dtinicio
...
--- Atualiza o Nível Referencia do Último Nível Referencia do Cargo Efetivo do Vínculo
update ecadHistNivelRefCEF
set nunivelenquadramento = &nivel,
    nureferenciaenquadramento = &referencia,
    nunivelpagamento = &nivel,
    nureferenciapagamento = &referencia    
where cdhistnivelrefcef = (select nivref.cdhistnivelrefcef
from (select cdvinculo from ecadvinculo where numatricula = &matricula and nuseqmatricula = &seq) v
inner join (select cdvinculo, max(dtinicio) as dtinicio from ecadHistCargoEfetivo group by cdvinculo) ultimocef on ultimocef.cdvinculo = v.cdvinculo
...
--- Atualização do Nivel Referencia da Capa de Pagamento com base nas informações do Arquivo de Migração da Capa de Pagamento
begin
  for i in (
with
matriculas as (
...
--- Atualização do Nivel Referencia da Relação de Vinculo do Efetivo com base na Capa de Pagamento
begin
  for i in (
select cef.cdhistcargoefetivo, cef.cdvinculo, cef.nunivelpagamento, cef.nureferenciapagamento, capa.nunivelcef, capa.nureferenciacef
...
where cef.nunivelpagamento != capa.nunivelcef
   or cef.nureferenciapagamento != capa.nureferenciacef
  ) loop

    update ecadHistCargoEfetivo set nunivelpagamento = i.nunivelcef, nureferenciapagamento = i.nureferenciacef
...
--- Atualização do Nivel Referencia da Hsitorico de Nivel Referencia da Realação de Vinculo do Efetivo com base na Capa de Pagamento
begin
  for i in (
select nivref.cdhistnivelrefcef, capa.nunivelcef, capa.nureferenciacef
from ecadHistNivelRefCEF nivref
...
inner join epagfolhapagamento f on f.flcalculodefinitivo = 'S' and f.cdtipofolhapagamento = 2 and cdtipocalculo = 1
                               and f.nuanoreferencia = 2022 and f.numesreferencia = 08
inner join epagcapahistrubricavinculo capa on capa.cdfolhapagamento = f.cdfolhapagamento and capa.cdvinculo = cef.cdvinculo
where nivref.nunivelpagamento != capa.nunivelcef
   or nivref.nureferenciapagamento != capa.nureferenciacef
  ) loop

    update ecadHistNivelRefCEF
       set nunivelenquadramento = i.nunivelcef, nureferenciaenquadramento = i.nureferenciacef, 
           nunivelpagamento = i.nunivelcef, nureferenciapagamento = i.nureferenciacef
    where cdhistnivelrefcef = i.cdhistnivelrefcef;
