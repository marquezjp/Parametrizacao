...
select 
      ...
      case when (select count(*) from ecadhistcargoefetivo cef 
                  ...
                  and cef.flanulado = 'N'
                  and cef.dtinicio <= &p_ultimo_dia_mes 
                  and nvl(cef.dtfim, '31/12/2199') >= &p_primeiro_dia_mes 
                  and cef.cdrelacaotrabalho = 5
                  and not exists
                        (select 1 from ecadhistcargocom cco
                        where cco.cdvinculo = v.cdvinculo
                              and cco.flanulado = 'N'
                              and cco.dtinicio <= &p_ultimo_dia_mes 
                              and nvl(cco.dtfim, '31/12/2199') >= &p_primeiro_dia_mes 
                        )
            ) > 0 then 'EFETIVO'
            ...
            when (select count(*) from ecadhistfuncaochefia fuc 
                  ...
                  and fuc.flanulado = 'N'
                  and fuc.dtinicio <= &p_ultimo_dia_mes 
                  and nvl(fuc.dtfim, '31/12/2199') >= &p_primeiro_dia_mes 
                  and not exists
                  ...
                  and not exists
                        ...
                        where apo.cdvinculo = v.cdvinculo
                              and apo.flanulado = 'N'
                              and apo.flativa = 'S'
                              and apo.dtinicioaposentadoria <= &p_ultimo_dia_mes 
                              and nvl(apo.dtfimaposentadoria, '31/12/2199') >= &p_primeiro_dia_mes 
                        )
            ...
            when (select count(*) from ecadhistestagio est
                  ...
                  and est.flanulado = 'N'  
                  and est.dtinicio <= &p_ultimo_dia_mes 
                  and nvl(est.dtfim, '31/12/2199') >= &p_primeiro_dia_mes 
            ) > 0 then 'ESTAGIÁRIO'   
            ...
            when (select count(*) from epvdhistpensaoprevidenciaria pen
                  ...
                  and pen.flanulado = 'N'  
                  and pen.dtinicio <= &p_ultimo_dia_mes 
                  and nvl(pen.dtfim, '31/12/2199') >= &p_primeiro_dia_mes 
            ...
            when (select count(*) from epvdhistpensaonaoprev penesp
                  ...
                  and penesp.dtinicio <= &p_ultimo_dia_mes 
                  and nvl(penesp.dtfim, '31/12/2199') >= &p_primeiro_dia_mes 
      ...
      capa.vlproventos - nvl(capa.vldescontos, 0) Liquido,
      
      mt.demotivoafasttemporario  Afast_Temp_SemRemun,
      md.demotivoafastdefinitivo  Afast_Def_SemRemun
...
from epagcapahistrubricavinculo capa
      ...
      where nvl(v.dtdesligamento, '31/12/2099') >= &p_primeiro_dia_mes
