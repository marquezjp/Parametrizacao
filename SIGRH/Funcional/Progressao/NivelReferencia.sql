select
 ...
 case
  ...
  when trim(sigrh.orgao) != trim(t.orgao) then 'ORGAO DIFERENTE'
  ...
  when regexp_replace(upper(trim(sigrh.nome_completo)), '[[:space:]]+', chr(32))
    ...
    then 'NOME DIFERENTE'
  when  sigrh.nivel_referencia != nivel.NivelReferencia then 'HOUVE PROGRESSAO POS SIGRH'
  else ''
 end as Observacao

from tmpprogressao t
left join (
select
 o.sgorgao as ORGAO,
...
) nivel on nivel.MATRICULA = sigrh.MATRICULA
